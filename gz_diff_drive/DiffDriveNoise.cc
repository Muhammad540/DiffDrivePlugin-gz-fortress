...
class ignition::gazebo::systems::DiffDriveNoisePrivate
{
  ...
  /// \brief GT speed of left joint
  public: double leftJointSpeedGT{0};

  /// \brief GT speed of right joint
  public: double rightJointSpeedGT{0};
  ...
  /// \brief Diff drive odometry (ground truth)
  public: math::DiffDriveOdometry odomGT;

  /// \brief Diff drive odometry message publisher (ground truth)
  public: transport::Node::Publisher odomGTPub;

  public: double previousLeftPosGT{0.0};

  public: double previousRightPosGT{0.0};

  /// \brief Diff drive tf message publisher.
  public: transport::Node::Publisher tfPub;

  /// \brief Diff drive gt tf message publisher.
  public: transport::Node::Publisher tfGTPub;
  ...
  public: std::default_random_engine randomGenerator;

  /// \brief Noise params
  double baseNoiseStdDev = 0.0;
  double noiserate = 0.001;
};

//////////////////////////////////////////////////
DiffDriveNoise::DiffDriveNoise()
  : dataPtr(std::make_unique<DiffDriveNoisePrivate>())
{
}

//////////////////////////////////////////////////
void DiffDriveNoise::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    ignerr << "DiffDriveNoise plugin should be attached to a model entity. "
  ...
  this->dataPtr->odomGT.SetWheelParams(this->dataPtr->wheelSeparation,
      this->dataPtr->wheelRadius, this->dataPtr->wheelRadius);
  ...
  this->dataPtr->node.Subscribe(topic, &DiffDriveNoisePrivate::OnCmdVel,
      this->dataPtr.get());
  ...
  {
    this->dataPtr->node.Subscribe(enableTopic, &DiffDriveNoisePrivate::OnEnable,
        this->dataPtr.get());
  }
  ...
  std::vector<std::string> gtOdomTopics;
  if (_sdf->HasElement("odom_gt_topic"))
  {
      gtOdomTopics.push_back(_sdf->Get<std::string>("odom_gt_topic"));
  }
  gtOdomTopics.push_back("/model/" + this->dataPtr->model.Name(_ecm) +
      "/odometry_gt");
  auto gtOdomTopic = validTopic(gtOdomTopics);

  this->dataPtr->odomGTPub = this->dataPtr->node.Advertise<msgs::Odometry>(
      gtOdomTopic);
  ...
  std::string tfGTTopic{"/model/" + this->dataPtr->model.Name(_ecm) +
    "/tf"};
  if (_sdf->HasElement("tf_gt_topic"))
    tfTopic = _sdf->Get<std::string>("tf_gt_topic");
  this->dataPtr->tfGTPub  = this->dataPtr->node.Advertise<msgs::Pose_V>(
      tfTopic);
  ...
  ignmsg << "DiffDriveNoise subscribing to twist messages on [" << topic << "]"
         << std::endl;
  ignmsg << "ground truth odometry topic [" << gtOdomTopic << "]" << std::endl;
}

//////////////////////////////////////////////////
void DiffDriveNoise::PreUpdate(const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  IGN_PROFILE("DiffDriveNoise::PreUpdate");
...
//////////////////////////////////////////////////
void DiffDriveNoise::PostUpdate(const UpdateInfo &_info,
    const EntityComponentManager &_ecm)
{
  IGN_PROFILE("DiffDriveNoise::PostUpdate");
  // Nothing left to do if paused.
  if (_info.paused)
    return;
...
//////////////////////////////////////////////////
void DiffDriveNoisePrivate::UpdateOdometry(const UpdateInfo &_info,
    const EntityComponentManager &_ecm)
{
  IGN_PROFILE("DiffDriveNoise::UpdateOdometry");
  // Initialize, if not already initialized.
  if (!this->odom.Initialized() || !this->odomGT.Initialized())
  {
    this->odom.Init(std::chrono::steady_clock::time_point(_info.simTime));
    this->odomGT.Init(std::chrono::steady_clock::time_point(_info.simTime));
    return;
  }
  ...
  auto leftPosGT = this->previousLeftPosGT + (this->leftJointSpeedGT * dt);
  auto rightPosGT = this->previousRightPosGT + (this->rightJointSpeedGT * dt);

  this->previousLeftPosGT = leftPosGT;
  this->previousRightPosGT = rightPosGT;
  ...
  this->odomGT.Update(leftPosGT, rightPosGT,
      std::chrono::steady_clock::time_point(_info.simTime));
  ...
  msgs::Odometry msgGT;
  msgGT.mutable_pose()->mutable_position()->set_x(this->odomGT.X());
  msgGT.mutable_pose()->mutable_position()->set_y(this->odomGT.Y());

  math::Quaterniond orientationGT(0, 0, *this->odomGT.Heading());
  msgs::Set(msgGT.mutable_pose()->mutable_orientation(), orientationGT);

  msgGT.mutable_twist()->mutable_linear()->set_x(this->odomGT.LinearVelocity());
  msgGT.mutable_twist()->mutable_angular()->set_z(*this->odomGT.AngularVelocity());

  // Set the time stamp in the header
  msgGT.mutable_header()->mutable_stamp()->CopyFrom(
      convert<msgs::Time>(_info.simTime));

  // Set the frame id for ground truth
  auto frameGT = msgGT.mutable_header()->add_data();
  frameGT->set_key("frame_id");
  if (this->sdfFrameId.empty())
  {
    frameGT->add_value(this->model.Name(_ecm) + "/odom");
  }
  else
  {
    frameGT->add_value(this->sdfFrameId);
  }

  // Set the child frame id for ground truth
  if (this->sdfChildFrameId.empty())
  {
    if (linkName)
    {
      auto childFrameGT = msgGT.mutable_header()->add_data();
      childFrameGT->set_key("child_frame_id");
      childFrameGT->add_value(this->model.Name(_ecm) + "/" + *linkName);
    }
  }
  else
  {
    auto childFrameGT = msgGT.mutable_header()->add_data();
    childFrameGT->set_key("child_frame_id");
    childFrameGT->add_value(this->sdfChildFrameId);
  }

  // Construct the ground truth Pose_V/tf message
  msgs::Pose_V tfMsgGT;
  msgs::Pose *tfMsgPoseGT = tfMsgGT.add_pose();
  tfMsgPoseGT->mutable_header()->CopyFrom(*msgGT.mutable_header());
  tfMsgPoseGT->mutable_position()->CopyFrom(msgGT.mutable_pose()->position());
  tfMsgPoseGT->mutable_orientation()->CopyFrom(msgGT.mutable_pose()->orientation());

  this->odomGTPub.Publish(msgGT);
  this->tfGTPub.Publish(tfMsgGT);
}

//////////////////////////////////////////////////
void DiffDriveNoisePrivate::UpdateVelocity(const UpdateInfo &_info,
    const EntityComponentManager &/*_ecm*/)
{
  IGN_PROFILE("DiffDriveNoise::UpdateVelocity");
  ...
  // ground truth
  this->rightJointSpeedGT = this->rightJointSpeed;
  this->leftJointSpeedGT = this->leftJointSpeed;

  // adding gaussian noise to the joint speeds
  double currenttime = std::chrono::duration<double>(_info.simTime).count();
  double noisestddev = this->baseNoiseStdDev + this->noiserate * currenttime;
  std::normal_distribution<double> noisedist(0.0, noisestddev);
  // std::cout << "Added Noise: " << noisedist(this->randomGenerator) << std::endl;
  this->rightJointSpeed += noisedist(this->randomGenerator);
  this->leftJointSpeed += noisedist(this->randomGenerator);
}

//////////////////////////////////////////////////
void DiffDriveNoisePrivate::OnCmdVel(const msgs::Twist &_msg)
{
  std::lock_guard<std::mutex> lock(this->mutex);
  if (this->enabled)
  {
...
//////////////////////////////////////////////////
void DiffDriveNoisePrivate::OnEnable(const msgs::Boolean &_msg)
{
  std::lock_guard<std::mutex> lock(this->mutex);
...
IGNITION_ADD_PLUGIN(DiffDriveNoise,
                    System,
                    DiffDriveNoise::ISystemConfigure,
                    DiffDriveNoise::ISystemPreUpdate,
                    DiffDriveNoise::ISystemPostUpdate)

IGNITION_ADD_PLUGIN_ALIAS(DiffDriveNoise, "gz::sim::systems::DiffDriveNoise")

// TODO(CH3): Deprecated, remove on version 8
IGNITION_ADD_PLUGIN_ALIAS(DiffDriveNoise, "ignition::gazebo::systems::DiffDriveNoise")
